...
export default function MentorAssignmentsPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (res.error) {
      ...
      setDueDate('');
      fetchAssignments();
      setTimeout(() => { setShowCreateForm(false); setSuccess(null); }, 1500);
    }
  };
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 mt-1">Create and manage assignments for your mentees</p>
        </div>
        <div className="flex gap-2">
          <button onClick={fetchAssignments} className="p-2.5 bg-gray-100 rounded-lg text-gray-600 hover:bg-gray-200 transition-colors">
            <RefreshCw size={18} className={fetching ? 'animate-spin' : ''} />
          </button>
          <button
            onClick={() => setShowCreateForm(!showCreateForm)}
            className="flex items-center gap-2 px-4 py-2.5 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-lg font-medium text-sm hover:from-indigo-700 hover:to-purple-700 transition-all"
          >
      ...
      {showCreateForm && (
        <div className="bg-white rounded-2xl border-2 border-indigo-200 p-6">
          <h2 className="text-lg font-semibold text-gray-900 mb-4 flex items-center gap-2">
            <Plus size={20} className="text-indigo-600" /> Create Assignment for Mentees
          </h2>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1.5">Title *</label>
              <input type="text" placeholder="e.g., Career Exploration Research Project" value={title} onChange={(e) => setTitle(e.target.value)} required minLength={3}
            ...
            <div>
              ...
              <input type="text" placeholder="e.g., NCERT Class 10 Science, Chapter 5" value={ncertRef} onChange={(e) => setNcertRef(e.target.value)}
                className="w-full px-4 py-2.5 rounded-lg border border-gray-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 outline-none transition-all text-sm" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1.5">Marking Criteria</label>
              <textarea placeholder="e.g., Research: 10 marks, Presentation: 5 marks" value={markingCriteria} onChange={(e) => setMarkingCriteria(e.target.value)} rows={3}
                className="w-full px-4 py-2.5 rounded-lg border border-gray-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 outline-none transition-all text-sm resize-none" />
            </div>
            <div>
            ...
            {success && <div className="flex items-center gap-2 text-emerald-700 text-sm p-3 bg-emerald-50 rounded-lg border border-emerald-200"><CheckCircle size={16} /> Assignment &quot;<strong>{success.title}</strong>&quot; created!</div>}

            <div className="flex gap-3">
              <button type="submit" disabled={loading || !title.trim()}
                className="flex-1 flex items-center justify-center gap-2 py-2.5 bg-indigo-600 text-white rounded-lg font-medium text-sm hover:bg-indigo-700 disabled:opacity-50 transition-all">
      ...
      {fetching ? (
        ...
        <div className="bg-white rounded-2xl border border-gray-200 p-12 text-center">
          ...
          <p className="text-gray-500 text-sm">Click &quot;New Assignment&quot; to create tasks for your mentees.</p>
        </div>
      ) : (
